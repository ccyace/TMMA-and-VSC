...
class Diffusion(object):
    ...
    def sample(self):
        ...
        if self.args.ptq and getattr(self.args, "two_stage", False):
            logger.info("=" * 75)
            logger.info("启用两阶段 BRECQ + 两段 DDIM 采样")
            logger.info("=" * 75)
    ...
    def _resolve_group_id(self, stage_tag: str) -> int:
        if "group0" in stage_tag or "stage1" in stage_tag:
            return 0
        if "group1" in stage_tag or "stage2" in stage_tag:
            return 1
    ...
    def _calibrate_qnn_with_dataset(self, fp_model, cali_data_path, stage_tag):
        ...
        gc.collect()
        cali_xs, cali_ts = cali_data
        logger.info("[%s] 校准数据形状: %s %s", stage_tag, cali_xs.shape, cali_ts.shape)
        ...
        def recon_model(root_model, kwargs):
            for name, module in root_model.named_children():
                if isinstance(module, QuantModule):
                    if module.ignore_reconstruction:
                        continue
                    logger.info("[%s] layer recon: %s", stage_tag, name)
                    layer_reconstruction(qnn, module, **kwargs)
                elif isinstance(module, BaseQuantBlock):
                    if module.ignore_reconstruction:
                        continue
                    logger.info("[%s] block recon: %s", stage_tag, name)
                    block_reconstruction(qnn, module, **kwargs)
                else:
                    recon_model(module, kwargs)
    ...
    def _build_two_stage_seq(self):
        ...
        logger.info("DDIM 100-step(%s) 切分: 前半=%d步(%d->%d), 后半=%d步(%d->%d)",
                    self.args.skip_type,
        ...
        return first_half_seq, second_half_seq
    ...
    def two_stage_quantized_sampling(self, fp_model):
        ...
        if self.args.quant_act and getattr(self.args, "max_avg_json", None):
            self.max_avg_stats = load_max_avg_statistics(self.args.max_avg_json)
        else:
            self.max_avg_stats = None

        first_half_seq, second_half_seq = self._build_two_stage_seq()
        batch_size = self.config.sampling.batch_size

        # Stage 1: group0 BRECQ
        ...
        qnn_stage1 = self._calibrate_qnn_with_dataset(
            fp_model_stage1, self.args.cali_data_path_group0, "stage1_group0"
        )

        # 前半程采样到中间噪声状态（完整两阶段才预生成整批噪声）
        if getattr(self.args, "first_half_only", False):
            first_half_dir = os.path.join(self.args.logdir, "first_half_img")
            os.makedirs(first_half_dir, exist_ok=True)
            logger.info(
                "仅前半段模式：PNG 将按 batch 写入 %s（采样进行中即可看到文件递增）",
                first_half_dir,
            )
            qnn_stage1.eval()
            img_idx = 0
            with torch.no_grad():
                for i in tqdm.tqdm(
                    range(0, self.args.max_images, batch_size),
                    desc="前半段采样",
                ):
                    n = min(batch_size, self.args.max_images - i)
                    x = torch.randn(
                        n,
                    ...
                    xs, _ = generalized_steps(
                        x, first_half_seq, qnn_stage1, self.betas, eta=self.args.eta
                    )
                    batch_out = xs[-1].cpu()
                    ...
                    for j in range(vis.shape[0]):
                        tvu.save_image(
                            vis[j],
                            os.path.join(first_half_dir, f"{img_idx}.png"),
                        )
                        img_idx += 1
            logger.info("前半段完成，共保存 %d 张 PNG: %s", img_idx, first_half_dir)
        ...
        qnn_stage2 = self._calibrate_qnn_with_dataset(
            fp_model_stage2, self.args.cali_data_path_group1, "stage2_group1"
        )

        # 后半程继续去噪并保存最终图像
        final_images = self._run_partial_sampling(
            qnn_stage2, intermediate_images, second_half_seq, batch_size, self.args.eta, "后半段采样"
        )
        final_images = inverse_data_transform(self.config, final_images)
        for i in range(final_images.shape[0]):
            tvu.save_image(final_images[i], os.path.join(self.args.image_folder, f"{i}.png"))
        torch.save(final_images, os.path.join(self.args.logdir, "final_samples.pt"))
        logger.info("两阶段采样完成，最终图像数: %d", final_images.shape[0])
...
def get_parser():
    ...
    parser.add_argument(
        ...
        help="两阶段量化+采样：前50步用组0量化模型，后50步用组1量化模型"
    )
    parser.add_argument(
        "--first_half_only",
        action="store_true",
        help="仅执行组0 BRECQ + 前半段 DDIM 采样，并保存 max_images 张 PNG（不跑第二阶段、不写 intermediate .pt）",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="out/dntc_cali/dntc_cifar10_g0_allst_k50_m50.pt",
        help="两阶段第一组（前半程）BRECQ 校准集路径"
    )
    parser.add_argument(
        "--cali_data_path_group1",
        type=str,
        default="out/dntc_cali/dntc_cifar10_g1_allst_k50_m50.pt",
        help="两阶段第二组（后半程）BRECQ 校准集路径"
    )
    parser.add_argument(
