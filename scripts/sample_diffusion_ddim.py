...
from qdiff.recon_metrics import ReconMetricsRegistry, load_layer_whitelist, DEFAULT_LAYERS_FILE

logger = logging.getLogger(__name__)

_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_DEFAULT_CALI_CKPT = os.path.join(_REPO_ROOT, "cifar_w8a8_ckpt.pth")
    ...
    def sample(self):
        ...
        if self.args.ptq:
            if self.args.quant_mode == 'qdiff':
                ...
                aq_params = {'n_bits': args.act_bit, 'symmetric': args.a_sym, 'channel_wise': False,
                             'scale_method': 'max', 'leaf_param': args.quant_act}
                if self.args.resume:
                ...
                if self.args.resume:
                    ...
                    if self.args.resume_w:
                        ckpt = (self.args.cali_ckpt or "").strip()
                        if not ckpt or not os.path.isfile(ckpt):
                            raise FileNotFoundError(
                                f"--resume_w 需要有效 --cali_ckpt，当前: {ckpt!r}"
                            )
                        logger.info("从 ckpt 恢复权重（跳过权重 BRECQ）: %s", ckpt)
                        resume_cali_model(qnn, ckpt, cali_data, False, cond=False)
                    else:
                    ...
                    recon_plot_dir = getattr(self.args, "recon_plot_dir", "") or ""
                    recon_plot_every = int(getattr(self.args, "recon_plot_every", 10))
                    recon_plot_max_iters = int(getattr(self.args, "recon_plot_max_iters", 2000))
                    recon_layers_file = (
                        getattr(self.args, "recon_plot_layers_file", "") or DEFAULT_LAYERS_FILE
                    )
                    recon_layer_list = load_layer_whitelist(
                        getattr(self.args, "recon_plot_layers", "") or "",
                        recon_layers_file,
                    )
                    _stages_raw = (getattr(self.args, "recon_plot_stages", "") or "act").strip().lower()
                    if _stages_raw in ("all", "both"):
                        recon_plot_stages = {"weight", "act"}
                    else:
                        recon_plot_stages = {
                            s.strip() for s in _stages_raw.split(",") if s.strip() in ("weight", "act")
                        }
                    recon_registries: dict = {}
                    recon_stage = "weight"

                    def _get_recon_registry(stage: str):
                        if not recon_plot_dir or stage not in recon_plot_stages:
                            return None
                        if stage not in recon_registries:
                            recon_registries[stage] = ReconMetricsRegistry(
                                log_dir=recon_plot_dir,
                                stage=stage,
                                layer_whitelist=recon_layer_list,
                                log_every=recon_plot_every,
                                max_iters=recon_plot_max_iters,
                                layers_file=recon_layers_file,
                            )
                            logger.info(
                                "BRECQ 曲线 [%s]：记录 %d 个关键层 -> %s",
                                stage,
                                len(recon_layer_list),
                                os.path.join(recon_plot_dir, "recon_metrics_log.json"),
                            )
                        return recon_registries[stage]

                    def recon_model(model, prefix=""):
                        """
                        Block reconstruction. For the first and last layers, we can only apply layer reconstruction.
                        """
                        registry = _get_recon_registry(recon_stage)
                        for name, module in model.named_children():
                            full_name = f"{prefix}.{name}" if prefix else name
                            logger.info(f"{full_name} {isinstance(module, BaseQuantBlock)}")
                            metrics_logger = (
                                registry.create_logger(full_name) if registry else None
                            )
                            call_kwargs = dict(kwargs, metrics_logger=metrics_logger, recon_tag=full_name)
                            if isinstance(module, QuantModule):
                                if module.ignore_reconstruction is True:
                                    logger.info('Ignore reconstruction of layer {}'.format(full_name))
                                    continue
                                else:
                                    logger.info('Reconstruction for layer {}'.format(full_name))
                                    layer_reconstruction(qnn, module, **call_kwargs)
                            elif isinstance(module, BaseQuantBlock):
                                if module.ignore_reconstruction is True:
                                    logger.info('Ignore reconstruction of block {}'.format(full_name))
                                    continue
                                else:
                                    logger.info('Reconstruction for block {}'.format(full_name))
                                    block_reconstruction(qnn, module, **call_kwargs)
                            else:
        ...
        if getattr(self.args, "act_recon_only", False):
            logger.info("act_recon_only：跳过 DDIM 采样，仅完成激活重建与曲线记录")
            return
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="BRECQ 重建曲线输出目录；设为非空则按 --recon_plot_stages 记录",
    )
    parser.add_argument(
        "--recon_plot_stages",
        type=str,
        default="act",
        help="记录阶段：act（默认，仅激活 BRECQ）、weight、或 weight,act / both",
    )
    parser.add_argument(
        "--recon_plot_every",
        type=int,
        default=10,
        help="重建指标记录间隔（iteration）",
    )
    parser.add_argument(
        "--recon_plot_max_iters",
        type=int,
        default=2000,
        help="每层仅记录前 N 次迭代（用于绘制训练初期曲线；不影响实际 cali_iters）",
    )
    parser.add_argument(
        "--recon_plot_layers_file",
        type=str,
        default="",
        help="关键层列表 JSON（默认 configs/recon_key_layers.json，可手改）",
    )
    parser.add_argument(
        "--recon_plot_layers",
        type=str,
        default="",
        help="逗号分隔关键层名，覆盖/追加 layers_file（如 model.mid.block_1）",
    )
    parser.add_argument(
        "--cali_ckpt",
        type=str,
        default=_DEFAULT_CALI_CKPT,
        help="已校准 ckpt；配合 --resume_w 跳过权重 BRECQ",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="从 --cali_ckpt 恢复权重，跳过权重 BRECQ，仅跑激活重建",
    )
    parser.add_argument(
        "--act_recon_only",
        action="store_true",
        help="激活校准+曲线记录后退出，不进行 DDIM 采样",
    )
    parser.add_argument(
