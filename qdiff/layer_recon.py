...
def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor,
                         ...
                         metrics_logger=None, recon_tag: str = "layer"):
    """
    ...
    if not act_quant:
        ...
        if len(opt_params) == 0:
            logger.warning("Skip layer reconstruction: no weight parameters to optimize in this layer.")
            if not include_act_func:
        ...
        if len(opt_params) == 0:
            logger.warning("Skip layer reconstruction: no activation parameters to optimize in this layer.")
            if not include_act_func:
                layer.activation_function = org_act_func
            return
    ...
    cached_inps, cached_outs = save_inp_oup_data(
        model, layer, cali_data, asym, act_quant, 4, keep_gpu=False, cond=cond, is_sm=is_sm)
    if opt_mode != 'mse':
        cached_grads = save_grad_data(model, layer, cali_data, act_quant, batch_size=batch_size)
    else:
        cached_grads = None

    # 检查是否为timestep-aware模式（cached_inps是tuple）
    is_timestep_aware = isinstance(cached_inps, (list, tuple)) and len(cached_inps) == 2
    if is_timestep_aware:
        try:
            ...
            logger.warning(
                "Timestep context not available, but cached_inps is a tuple. Timestep-aware mode may not work correctly.")
    else:
        HAS_TIMESTEP_CONTEXT = False
    ...
    for i in range(iters):
        if is_timestep_aware:
            # cached_inps是(xs, ts)元组
            cached_xs, cached_ts = cached_inps
            idx = torch.randperm(cached_xs.size(0))[:batch_size]
            cur_inp = cached_xs[idx].to(device)
            cur_ts = cached_ts[idx].to(device)  # 获取对应的timesteps
            cur_out = cached_outs[idx].to(device)
            cur_grad = cached_grads[idx] if opt_mode != 'mse' and cached_grads is not None else None
        else:
            # 标准模式
        ...
        # 如果是timestep-aware模式，在上下文中调用layer
        if is_timestep_aware and HAS_TIMESTEP_CONTEXT:
            # 使用batch的第一个timestep（假设batch内timesteps相同）
            # cur_ts是1D tensor，取第一个元素作为代表
            if cur_ts.numel() > 0:
                batch_ts = cur_ts[0:1] if cur_ts.dim() >= 1 else cur_ts.unsqueeze(0)
            else:
                batch_ts = cur_ts
