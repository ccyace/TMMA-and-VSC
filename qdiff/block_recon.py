...
# 使用时间步感知版本的utils（向后兼容原始QuantModel）
try:
    from qdiff.timestep_aware_utils import save_grad_data, save_inp_oup_data
except ImportError:
...
def _opt_grad_l2_norm(opt_params) -> float:
    sq = 0.0
    for p in opt_params:
...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor,
                         ...
                         metrics_logger=None, recon_tag: str = "block"):
    """
    ...
    if not act_quant:
        ...
        if len(opt_params) == 0:
            logger.warning("Skip block reconstruction: no weight parameters to optimize in this block.")
            if not include_act_func:
        ...
        if len(opt_params) == 0:
            logger.warning("Skip block reconstruction: no activation parameters to optimize in this block.")
            if not include_act_func:
                block.activation_function = org_act_func
            return
    ...
    cached_inps, cached_outs = save_inp_oup_data(
        model, block, cali_data, asym, act_quant, 4, keep_gpu=False, cond=cond, is_sm=is_sm)
    if opt_mode != 'mse':
        cached_grads = save_grad_data(model, block, cali_data, act_quant, batch_size=batch_size)
    else:
        cached_grads = None

    # 检查是否为timestep-aware模式
    # cached_inps可能是:
    # 1. Tensor - 标准模式
    # 2. List[Tensor, Tensor] - 注意力块 [x, temb]（旧格式）
    # 3. Tuple[Tensor, Tensor] - timestep-aware 层 (xs, timesteps)
    # 4. Dict - timestep-aware 注意力块 {'inputs': [x, temb], 'timesteps': ts}
    is_timestep_aware = False
    cached_timesteps_dict = None

    if isinstance(cached_inps, dict) and 'timesteps' in cached_inps:
        # 注意力块的timestep-aware格式
        is_timestep_aware = True
        cached_timesteps_dict = cached_inps['timesteps']
        cached_inps = cached_inps['inputs']  # 恢复为 [x, temb]
    elif isinstance(cached_inps, tuple) and len(cached_inps) == 2 and not isinstance(cached_inps, list):
        # 普通层的timestep-aware格式
        is_timestep_aware = True

    if is_timestep_aware:
        try:
            from qdiff.timestep_context import timestep_context
            HAS_TIMESTEP_CONTEXT = True
        except ImportError:
            HAS_TIMESTEP_CONTEXT = False
            logger.warning("Timestep context not available for block reconstruction")
    else:
        HAS_TIMESTEP_CONTEXT = False
    ...
    for i in range(iters):
        ...
        if cached_timesteps_dict is not None:
            # Timestep-aware 注意力块: cached_inps 是 [x, temb], timesteps 在字典中
            idx = torch.randperm(cached_inps[0].size(0))[:batch_size]
            cur_x = cached_inps[0][idx].to(device)
            cur_t = cached_inps[1][idx].to(device)
            cur_inp = (cur_x, cur_t)
            cur_ts = cached_timesteps_dict[idx].to(device)
            cur_out = cached_outs[idx].to(device)
            cur_grad = cached_grads[idx].to(device) if opt_mode != 'mse' and cached_grads is not None else None
        elif isinstance(cached_inps, tuple) and not isinstance(cached_inps, list):
            # Timestep-aware 普通层: cached_inps 是 (xs, ts) tuple
            cached_xs, cached_ts = cached_inps
            idx = torch.randperm(cached_xs.size(0))[:batch_size]
            cur_inp = cached_xs[idx].to(device)
            cur_ts = cached_ts[idx].to(device)
            cur_out = cached_outs[idx].to(device)
            cur_grad = cached_grads[idx].to(device) if opt_mode != 'mse' and cached_grads is not None else None
        elif isinstance(cached_inps, list):
            # 标准注意力块: cached_inps 是 [x, temb] list（非timestep-aware）
            idx = torch.randperm(cached_inps[0].size(0))[:batch_size]
            cur_x = cached_inps[0][idx].to(device)
            cur_t = cached_inps[1][idx].to(device)
            cur_inp = (cur_x, cur_t)
        ...
        # 如果是timestep-aware模式且有timesteps，在上下文中调用block
        if is_timestep_aware and HAS_TIMESTEP_CONTEXT and cur_ts is not None:
            batch_ts = cur_ts[0:1] if cur_ts.numel() > 0 else cur_ts
            with timestep_context(timestep=batch_ts):
        ...
        if metrics_logger is not None:
            cur_lr = scheduler.get_last_lr()[0] if scheduler else None
            metrics_logger.log(
                iter_idx=i,
                total=loss_func.last_total,
                rec=loss_func.last_rec,
                round_loss=loss_func.last_round,
                beta=loss_func.last_beta,
                grad_norm=_opt_grad_l2_norm(opt_params),
                lr=cur_lr,
            )
        if multi_gpu:
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, grad=None):
        ...
        self.last_total = float(total_loss.detach().cpu())
        self.last_rec = float(rec_loss.detach().cpu())
        self.last_round = float(round_loss.detach().cpu() if hasattr(round_loss, "detach") else round_loss)
        self.last_beta = float(b)
        if self.count % 500 == 0:
            logger.info('Total loss:\t{:.3f} (rec:{:.3f}, round:{:.3f})\tb={:.2f}\tcount={}'.format(
                self.last_total, self.last_rec, self.last_round, self.last_beta, self.count))
        return total_loss
