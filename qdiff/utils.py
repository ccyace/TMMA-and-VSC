    ...
    for i in trange(num):
        ...
        if isinstance(cur_inp, tuple):
            ...
            if not is_sm:
                ...
                cached_inps.index_copy_(0, torch.arange(i * cur_inp.shape[0], (i + 1) * cur_inp.shape[0]),
                                        cur_inp.cpu())
...
def get_train_samples(args, sample_data, custom_steps=None):
    ...
    if num_st == 1:
        ...
        if isinstance(sample_data, dict):
            # 扁平字典：xs/ts 均为 Tensor，逐样本时间步（如 DNTC 生成的 .pt）
            if torch.is_tensor(sample_data.get("xs")) and torch.is_tensor(sample_data.get("ts")):
                xs = sample_data["xs"][:num_samples]
                ts = sample_data["ts"][:num_samples].float().reshape(-1)
            elif "xs" in sample_data:
                if isinstance(sample_data["xs"], (list, tuple)):
                    xs = sample_data["xs"][0][:num_samples]
                    if "ts" in sample_data:
                        if isinstance(sample_data["ts"], (list, tuple)) and len(sample_data["ts"]) > 0:
                            ts = sample_data["ts"][0][:num_samples].float().reshape(-1)
                        elif torch.is_tensor(sample_data["ts"]):
                            ts = sample_data["ts"][:num_samples].float().reshape(-1)
                else:
                    xs = sample_data["xs"][:num_samples]
                    if torch.is_tensor(sample_data.get("ts")):
                        ts = sample_data["ts"][:num_samples].float().reshape(-1)
            if xs is None and "xs" not in sample_data:
                first_key = list(sample_data.keys())[0]
                data = sample_data[first_key]
                if isinstance(data, (list, tuple)):
                    xs = data[0][:num_samples] if len(data) > 0 else data[:num_samples]
                else:
                    xs = data[:num_samples]
        else:
        ...
        if ts is None:
            n = min(num_samples, xs.size(0))
            xs = xs[:n]
            ts = torch.ones(n, dtype=torch.float32, device=xs.device) * 800.0
        else:
            n = min(num_samples, xs.size(0), ts.size(0))
            xs = xs[:n]
            ts = ts[:n]
        if xs.size(0) != ts.size(0):
            raise ValueError(
                f"cali_st=1: xs 与 ts 样本数不一致: {xs.size(0)} vs {ts.size(0)}"
            )
    else:
        ...
        if isinstance(sample_data, dict):
            ...
            raise ValueError(f"当 num_st > 1 时，sample_data 必须是字典格式，但得到 {type(sample_data)}")
        ...
        if args.cond:
            xs_lst += xs_lst
            ts_lst += ts_lst
            conds_lst = [sample_data["cs"][i][:num_samples] for i in timesteps] + [sample_data["ucs"][i][:num_samples]
                                                                                   for i in timesteps]
        ...
        if args.cond:
            ...
            if ts.dtype != torch.float32:
                ts = ts.float()
            return xs, ts, conds
    if torch.is_tensor(xs) and xs.dtype != torch.float32:
        xs = xs.float()
    if torch.is_tensor(ts) and ts.dtype != torch.float32:
        ts = ts.float()
    return xs, ts
    ...
    for name, module in model.named_children():
        if isinstance(module, QuantModule):
            if module.ignore_reconstruction is True:
                ...
                module.weight_quantizer = AdaRoundQuantizer(uaq=module.weight_quantizer,
                ...
                for name, sub_module in module.named_modules():
                    if isinstance(sub_module, QuantModule):
                        if sub_module.split != 0:
                            # print(f"split {name}")
                            sub_module.weight_quantizer = AdaRoundQuantizer(uaq=sub_module.weight_quantizer,
                                                                            round_mode='learned_hard_sigmoid',
                                                                            weight_tensor=sub_module.org_weight.data[:,
                                                                                          :sub_module.split, ...])
                            sub_module.weight_quantizer_0 = AdaRoundQuantizer(uaq=sub_module.weight_quantizer_0,
                                                                              round_mode='learned_hard_sigmoid',
                                                                              weight_tensor=sub_module.org_weight.data[
                                                                                            :, sub_module.split:, ...])
                        else:
